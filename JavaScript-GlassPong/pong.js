...
var keys = {};
var gameState = 0; // 0: başlangıç, 1: çalışıyor, 2: boşta, 3: kazan, 4:bitti
...
function input() {
    ...
    if (38 in keys) { //Yukarı ok tuşuna basılırsa
        if (player1.y - player1.gravity > 0) {
            player1.y -= player1.gravity;
        }
    } else if (40 in keys) { //Aşağı ok tuşuna basılırsa
...
function Box(options){ /*Box nesnesinin kurucusu*/
    this.x = options.x || 10;
    this.y = options.y || 10;
...
var player1 = new Box({ /*oyuncu kürek*/
    x: 20,
...
var player2 = new Box({ /*oyuncu kürek*/
...
function ballBounce() {
	...
	if (((theBall.y+theBall.gravity) <= 0) || ((theBall.y+theBall.gravity+theBall.height) >= canvas.height)){
        theBall.gravity = (theBall.gravity) * (-1); /*Varsa, ters yönde hareket etmek için yerçekimi değerini değiştirin*/
        theBall.y += theBall.gravity; // Topu Hareket Ettir
        theBall.x += theBall.speed;
    } else { // Top yukarıya veya aşağıya çarpmazsa, Topu normal şekilde hareket ettirin
        theBall.x += theBall.speed;
...
function drawBox(box) { /* her kutuyu çiz*/
    ctx.fillStyle = box.color;
    ctx.fillRect(box.x, box.y, box.width, box.height);
}

function displayScore1() { /* player1 skor*/
    ctx.font = "20px Arial";
...
function displayScore2() { /*player2 skor*/
    ctx.font = "20px Arial";
...
function draw() {
    ...
    if(gameState==1 || gameState==2){ // çalışma/ilerleme
...
function loop() { /*Döngü fonksiyonu sürekli olarak çizim fonksiyonunu çağıracak ve ekranımızı güncelleyecektir*/
    if(gameState == 0){
        init();
    }
