...
function pice(val) {
	...
	var j = 0; //hücre stili için
	//diğer yolları temizliyoruz
	for (var i = 1; i <= 8 * 8; i++) {
	...
	if (id.innerHTML != "") {
		...
		if (pcolor == "black" && turn[turnval] == pturn) {
			...
			if (pname == "pawn") {
				...
				if (val > 8 && val <= 16) {
					// aynı anda üç hücreyi de almak için döngü
					for (var i = 0; i < 2; i++) {
						j += 8;
						document.getElementById("b" + val).style.filter = "brightness(1)";
						//sonraki hücrenin kontrol edilmesi
			...
			if (pname == "rook" || pname == "queen") {
				...
				j = 1;
				var moveright = 0;
				//sağ kenarda değil kontrol ediliyor
				if (val % 8 == 0) {
					moveright = 0
				} else {
					moveright = 8 - (val % 8);
				...
				//sola hareket{
				j = -1;
				//sol kenarda değil kontrol ediliyor
				var moveleft = 0;
				if (val % 8 == 0) {
					moveleft = 7;
				} else {
		...
		} //siyah hamle kontrolünün sonu
		//beyaz parça kontrol ediliyor
			...
			if (pname == "pawn") {
				...
				if (val < 57 && val > 48) {
					...
					for (var i = 0; i < 2; i++) {
						j -= 8;
						document.getElementById("b" + val).style.filter = "brightness(1)";
						//sonraki hücrenin kontrol edilmesi boş
						if (document.getElementById("b" + (val + j)).classList[2] == "none") {
							document.getElementById("b" + (val + j)).style.filter = "brightness(1)";
							arry.push(val + j);
						}
		...
		} //beyaz resim kontrolünün sonu
	} //boş hücrelerin kontrol edilmesinin sonu
	move(val);
}
